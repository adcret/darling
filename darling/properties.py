...
.. code-block:: python
    ...
    mean, covariance = properties.moments(data, coordinates=(phi, chi))
...
def mean(data, coordinates):
    ...
    .. code-block:: python
        ...
        first_moment = properties.mean(data, coordinates=(phi, chi))
    ...
    assert data.dtype==np.dtype.uint16, 'data must be of type uint16'
...
def covariance(data, coordinates, first_moments=None):
    ...
    .. code-block:: python
        ...
        covariance = properties.covariance(data, (phi, chi), first_moments=first_moment)
    ...
    _second_moments(data, first_moments, m1_mesh, m2_mesh, points, dum, res)
    return res

@numba.guvectorize([(numba.uint16[:, :], numba.float32[:,:], numba.float32[:,:], numba.float32[:], numba.float32[:])], '(n,m),(n,m),(n,m),(p)->(p)', nopython=True, target='parallel')
...
@numba.guvectorize([(numba.uint16[:, :], numba.float32[:], numba.float32[:,:], numba.float32[:], numba.float32[:,:])], '(n,m),(p),(k,q),(p)->(p,p)', nopython=True, target='parallel')
