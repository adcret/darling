...
class EnergyScan(Reader):
    ...
    def __call__(self, data_name, scan_id, roi=None):
        ...
        with h5py.File(self.abs_path_to_h5_file, "r") as h5f:
            ...
            chi = h5f[key0 + self.motor_names[1]][:].astype(np.float32)

            _, det_rows, det_cols = h5f[key0 + data_name].shape
            ...
            for i, key in enumerate(h5f.keys()):  # iterates over energies.
                ...
                energy[i] = h5f[key + self.motor_names[0]][()]
                all_chis.extend(list(h5f[key0 + self.motor_names[1]][:]))
