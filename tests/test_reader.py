...
def create_mock_dataset_with_bad_motors():
    ...
    with h5py.File(file_path, "w") as h5f:
        ...
        # Add motor values that don't align with expected precision
        ...
        # Add a data array with mismatched dimensions
...
class TestWithBadMotors(unittest.TestCase):
    ...
    def test_scan_size_fallback(self):
        ...
        # Use a scan_size to fix the issue
        scan_size = [100, 50]
        data, motors = reader(
            data_name=self.data_name,
            scan_id="1.1",
        ...
        # Assert that motors have been adjusted correctly
        self.assertEqual(len(motors[0]), scan_size[0])
        self.assertEqual(len(motors[1]), scan_size[1])

        # Assert that data shape matches the adjusted motors
        self.assertEqual(data.shape[2], scan_size[0])
        self.assertEqual(data.shape[3], scan_size[1])
